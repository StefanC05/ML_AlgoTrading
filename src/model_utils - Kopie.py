...
class PurgeTimeSeriesSplit:
    ...
    def split(self, df: pd.DataFrame, n_splits: int = 5):
        ...
        while fold_idx < n_splits and current_test_start_idx + TEST_SIZE_DAYS < n_dates:
            # Training window ends at test start (minus purge)
...
def train_dataPrep(df: pd.DataFrame) -> pd.DataFrame:
    """
    Later it should use a more robust and felxible way, to "reduce" extrem values.
...
def save_results(
    ...
    oos_preds_final.to_hdf("oos_preds.h5", key="predictions", mode="w")
    print(f"Predictions: oos_preds.h5 ({len(oos_preds_final):,} rows)")

    # 2. Per-Fold Detailed Results
    df_fold_results = pd.DataFrame(fold_results)
    df_fold_results.to_csv("training_fold_results.csv", index=False)
    print(
        f" Per-fold results. FIle: training_fold_results.csv ({len(df_fold_results)} )"
    )
    ...
    summary_stats.to_csv("training_stats_summary.csv", index=False)
    print(f"\n Summary statistics save: training_stats_summary.csv")

    df_stats.to_csv("training_stats_per_fold.csv", index=False)
    print(f"\n Per-fold statistics saved: training_stats_per_fold.csv")

    df_timing = pd.DataFrame(timing_stats)
    df_timing.to_csv("training_timing_stats.csv", index=False)
    print(f"\n Timing statistics saved: training_timing_stats.csv")

    joblib.dump(models_final, "models_final.joblib")
    print(
        f" Models saved: models_final.joblib ({len(models_final)} models from last fold)"
    )
